...
--function CrashGuard()
--
--	local CrashNumb = self:GetClientNumber( "precision_nudge" )
--	local CrashMax = 1010
    
 --   if CrashNumb > CrashMax then
--
  --  	print( "Crash Attempt Detected" )

 --       RunConsoleCommand( "precision_nudge", "10" )
					...
					if ( CurrentEnt:GetPhysicsObjectCount() < 2 ) then //not a ragdoll
						...
						elseif ( mode == 10 && !self.RepairTodo ) then//Repair spaz
							...
							elseif ( CurrentEnt:GetCollisionGroup() == COLLISION_GROUP_NONE ) then
								...
								//CurrentPhys:EnableGravity( !CurrentPhys:IsGravityEnabled() )	
...
function TOOL:LeftClick( trace )
	...
	if ( stage == 0 ) then//first click - choose a target.
		...
		if ( mode == 1 || mode == 10 || mode == 11 ) then 
