...
if (CLIENT) then
	...
	language.Add( "Tool.keypad.desc", "Keypad tool: Protect your stuff." )
	language.Add( "Tool.keypad.0", "Left Click: Spawn a Keypad, Right Click: Update Keypad with settings" )

	language.Add( "Undone_keypad", "Undone Keypad" )
	language.Add( "Cleanup.keypads", "Keypads" )
	language.Add( "Cleaned.keypads", "Cleaned up all Keypads" )
...
function TOOL:SetupKeypad(Ent, Password)
	...
	if self:GetClientNumber("length1") >= 500 then self:GetOwner():ConCommand("keypad_length1 500") end
	Ent:SetNetworkedInt("keypad_length1", self:GetClientNumber("length1"))
	if self:GetClientNumber("length2") >= 120 then self:GetOwner():ConCommand("keypad_length2 120") end
...
function TOOL.BuildCPanel( CPanel )
	CPanel:AddControl( "Header", {	Text = "#Tool.keypad.name", Description	= "#Tool.keypad.desc" }  )

	CPanel:AddControl( "TextBox", {	Label		= "Password",
									MaxLength	= "4",
									Command		= "keypad_password" })
