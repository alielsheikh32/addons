...
function ENT:Draw()
	...
	local broken = self.Entity:GetNWBool("keypad_broken")
	...
	cam.Start3D2D(pos, ang, 0.05)
		...
		local Num = self.Entity:GetNWString("keypad_num")
		local Access = self.Entity:GetNWBool("keypad_access")
		local ShowAccess = self.Entity:GetNWBool("keypad_showaccess")
		local Secure = self.Entity:GetNWBool("keypad_secure")
		...
		if broken then
			draw.DrawText("ERROR","Trebuchet34", X+7, Y+7, Color(255, 0, 0, 255))
		elseif (string.len(Num) ~= 0) and not (ShowAccess) then
			if (Secure) then
...
function ENT:Break()
	...
	effectdata:SetOrigin( self:GetPos() - self:GetUp() * -5 + self:GetForward() * .5)
	...
	local startposrand = 3
	local em = ParticleEmitter(self:GetPos())
	for i=1, 250 do
		 local part = em:Add("trails/smoke", self:GetPos() + (self:GetForward()) + (self:GetUp() * 2) + Vector(math.Rand(-startposrand, startposrand), math.Rand(-startposrand, startposrand), math.Rand(-startposrand, startposrand)))
		 if part then
			local bright = math.random(40, 50)
			local vel = 1
			  part:SetColor(bright,bright,bright)
			  part:SetStartAlpha(25)
			  part:SetVelocity(Vector(math.random(-vel,vel),math.random(-vel,vel),math.random(-vel, vel)) + (self:GetForward() * 2) + (self:GetUp() * math.random(.5, 10)))
			  part:SetAngles(Angle(math.random(0, 360), math.random(0, 360), math.random(0, 360)))
			  part:SetDieTime(math.random(2, 5))
			  part:SetStartSize(math.random(.5, 1))
			  part:SetEndSize(math.random(2, 3))
		 end
	end
	em:Finish()

	self:EmitSound("ambient/energy/spark"..tostring(math.random(1, 6))..".wav", 80, 120)
	
	timer.Simple(math.random(1, 4), function()
		if IsValid(self) and self:GetNWBool("keypad_broken") then
			self:Break()
		end
	end)
end

local function keypadBreak(um)
	local keypad = um:ReadEntity()
	if IsValid(keypad) then
		keypad:Break()
	end
end
usermessage.Hook("keypad_break", keypadBreak)